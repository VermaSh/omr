...
TR::Node* generateLenForArrayCopy(TR::Compilation *comp, int32_t elementSize, TR::Node *stride,TR::Node *srcObjNode,TR::Node *copyLenNode,TR::Node *n)
   ...
   if (elementSize == 1)
      ...
      len = TR::TransformUtil::generateOffsetNode(comp, copyLenNode, stride, elementSize, true);
      }
   else
      {
      len = TR::TransformUtil::generateOffsetNode(comp, copyLenNode, stride, elementSize, false);
      }
...
TR::Node *generateArrayAddressTree(
   ...
   if (offHigh > 0)
      ...
      if (elementSize == 1)
         ...
         offsetNode = TR::TransformUtil::generateOffsetNode(comp, offNode, stride, elementSize, true);
         }
      else
         {
         offsetNode = TR::TransformUtil::generateOffsetNode(comp, offNode, stride, elementSize, false);
         }
      }
   // else, there is not offset so we just need to get address of the first array element

   arrayElementAddressNode = TR::TransformUtil::generateArrayAddressNode(comp, objNode, offsetNode);
...
*/
   ...
   if (encode)
      {
      srcPosition = TR::TransformUtil::generateOffsetNode(comp(), srcOff, strideNode, 0, false);
      arrayTranslateNode->setSourceIsByteArrayTranslate(false);
      arrayTranslateNode->setTargetIsByteArrayTranslate(true);
      }
   else
      {
      dstPosition = TR::TransformUtil::generateOffsetNode(comp(), dstOff, strideNode, 0, false);
      arrayTranslateNode->setSourceIsByteArrayTranslate(true);
      arrayTranslateNode->setTargetIsByteArrayTranslate(false);
      }
   src = TR::TransformUtil::generateArrayAddressNode(comp(), srcObj, srcPosition);
   dst = TR::TransformUtil::generateArrayAddressNode(comp(), dstObj, dstPosition);
...
void OMR::ValuePropagation::transformArrayCloneCall(TR::TreeTop *callTree, OMR::ValuePropagation::ArrayCloneInfo *cloneInfo)
   ...
   TR::Node *lengthInBytes = TR::TransformUtil::generateOffsetNode(comp(), lenNode, NULL, elementSize, false);
